...
static char args_doc[] = "TARGET_FILE";

/* Flags for different command configurations */
static struct argp_option options[] = {
  {"verbose",      'v', 0,         0, "Produce verbose output"},
  {"quiet",        'q', 0,         0, "Don't produce any output"},
  {"dictionary",   'd', "FILE",    0, "Use dictionary attack with specified wordlist or dictionary file for rainbow table generation"},
...
struct arguments parse_arguments(int argc, char **argv) {
  ...
  arguments.username_prefix = NULL;
  ...
  if (arguments.mode == MODE_NONE && !arguments.benchmark) {
    ...
    if (arguments.dictionary_file == NULL) {
      arguments.dictionary_file = DICTIONARY_FILE;
