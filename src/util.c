...
static int check_crypt_generic(const char *password,
                               const char *stored_hash)
{
    struct crypt_data data = {0};
    char *result = crypt_r(password, stored_hash, &data);
    return result && strcmp(result, stored_hash) == 0;
}

int check_md5_crypt   (const char *pwd, const char *hash) { return check_crypt_generic(pwd, hash); }
int check_sha256_crypt(const char *pwd, const char *hash) { return check_crypt_generic(pwd, hash); }
int check_bcrypt_crypt(const char *pwd, const char *hash) { return check_crypt_generic(pwd, hash); }
